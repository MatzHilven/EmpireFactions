...
public class FactionWithdrawSubCommand implements SubCommand {

    private final EmpireFactions main;

    public FactionWithdrawSubCommand(EmpireFactions main) {
        this.main = main;
    }
    ...
    public void onCommand(CommandSender sender, Command command, String[] args) {
        ...
        if (!faction.removeMoney(amount)) {
            StringUtils.sendMessage(sender, Messager.NOT_ENOUGH_IN_BANK);
            return;
        }

        main.getEcon().depositPlayer(player, amount);
        StringUtils.sendMessage(sender, Messager.WITHDRAWN.replace("%amount%", StringUtils.format(amount)));
    }
    ...
    public String getUsage() {
        return Messager.USAGE_WITHDRAW;
    }
