...
public class FactionAllowAllySubCommand implements SubCommand {

    private final EmpireFactions main;

    public FactionAllowAllySubCommand(EmpireFactions main) {
        this.main = main;
    }
    ...
    public void onCommand(CommandSender sender, Command command, String[] args) {
        ...
        Optional<Faction> oFaction = empire.getFaction(player);
        ...
        Faction playerFaction = oFaction.get();
        ...
        if (playerFaction.getUniqueId().toString().equals(faction.getUniqueId().toString())) {
            StringUtils.sendMessage(sender,Messager.INVALID_FACTION_SELF);
            return;
        }

        if (playerFaction.addAllowedAlly(faction)) {
            StringUtils.sendMessage(sender, Messager.ALLOWED_ALLY.replace("%faction%", faction.getNameColored()));
        } else {
            StringUtils.sendMessage(sender, Messager.ALREADY_ALLY);
        }
    }
    ...
    public String getUsage() {
        return Messager.USAGE_ALLOW_ALLY;
    }
}
